...
function populate() {	
	...
	function download_clan_stats() {
		$.when(
			$.Deferred(function() {
				var self = this;
				get_wg_clan_data("/clans/info/?", ["members.account_id","members.account_name","members.role_i18n","members.role"], function(data) {
					clan_data = data;
					self.resolve();
				});
			}),
			...
			$( document ).ready(function() {
				...
				for (var i in clan_data.members) {
					...
					if (member_stats) {
						...
						if (Object.keys(member_stats[interesting_points[1]]).length > 0) {
		...
		$.get(link, {}, function(data) {
			if (data.data.length == 0) {
				...
				if (data.data[0].nickname == $('#srch-term').val() || data.data.length == 1) {
					clan = data.data[0].account_id
					reset_ui();
					populate();
				} else {
					var alternatives = "";
					for (var i = 0; i < Math.min(20, data.data.length); i++) {
						alternatives += "<a href='/clan/"+ data.data[i].account_id + "'>" + data.data[i].nickname + "</a>, "
					}
